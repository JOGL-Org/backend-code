...
foreach (var file in Directory.GetFiles("xrp/"))
{
    try
    {
        var jsonString = File.ReadAllText(file);
        var json = JsonNode.Parse(jsonString);
        var firstName = json["author"]["First Name"]?.GetValue<string>();
        var lastName = json["author"]["Last Name"]?.GetValue<string>();
        ...
        var user = new User
        {
            FirstName = json["author"]["First Name"].GetValue<string>(),
            LastName = json["author"]["Last Name"].GetValue<string>(),
            ShortBio = json["author"]["Headline"]?.GetValue<string>(),
            Bio = json["author"]["Bio"]?.GetValue<string>(),
            Experience = json["Experience"]?.Deserialize<List<UserExperience>>(),
            Education = json["Education"]?.Deserialize<List<UserEducation>>(),
            Username = json["author"]["First Name"].GetValue<string>() + json["author"]["Last Name"].GetValue<string>(),
            Email = $"{json["author"]["First Name"].GetValue<string>()}.{json["author"]["Last Name"].GetValue<string>()}+xrp@jogl.io"
        };
        ...
        var repos = json["Repo_list"]?.AsArray();
